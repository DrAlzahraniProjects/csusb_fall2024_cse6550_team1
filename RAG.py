...
class BackendClient():
    ...
    def query_rag(self,query):
        ...
        try:
            ...
            query_embedding = np.array(self.model.encode(query), dtype=np.float32).tolist()
            ...
            if not retrieved_documents:
                self.logger.info("No Relevant Documents Retrieved, so sending default response")
            ...
            self.logger.info("Most Relevant Document Retrieved")
            ...
            self.logger.error(f"HTTPStatusError: {e}")
    ...
    def get_existing_hashes_from_db(self,collection: Collection):
        """
    ...
    async def _load_documents_from_web_and_db(self,collection: Collection):
        """
        ...
        with ThreadPoolExecutor() as pool:
            ...
            documents_future = loop.run_in_executor(pool, self.load_documents_from_web)

            hashes_future = loop.run_in_executor(pool, self.get_existing_hashes_from_db, collection)

            documents, existing_hashes = await asyncio.gather(documents_future, hashes_future)

            return documents, existing_hashes

    def initialize_milvus(self,uri: str=MILVUS_URI):
        """
        ...
        if self.vector_store_check(uri):
            collection = Collection(re.sub(r'\W+', '', CORPUS_SOURCE))
            documents, existing_hashes = asyncio.run(self._load_documents_from_web_and_db(collection))

            # Split the documents into chunks
            docs = self.split_documents(documents=documents)
            ...
            for doc in docs:
                ...
                if hashed_text in existing_hashes:
                    ...
                    self.logger.info("Hashed Text insert")
            ...
            if existing_hashes:
                ...
                self.logger.info("Deleted outdated documents")
            self.create_vector_store(documents_to_insert)
        else:
            documents = self.load_documents_from_web()
            self.logger.info("Documents Loaded")
            # Split the documents into chunks
            docs = self.split_documents(documents=documents)
    ...
    def load_documents_from_web(self):
        ...
        for doc in raw_documents:
            cleaned_text = self.clean_text_from_html(doc.page_content)
            cleaned_documents.append(Document(page_content=cleaned_text, metadata=doc.metadata))

        return cleaned_documents

    def clean_text_from_html(self,html_content):
        """
    ...
    def vector_store_check(self,uri):
        """
    ...
    def create_vector_store(self,docs):
        ...
        count = 0
        for doc in docs:
            self.logger.info(f"Inserting Document {count}")
            text = doc.page_content[:MAX_TEXT_LENGTH]
